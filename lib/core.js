...
import {vueStringify} from './plugin/vue-stringify.js'
...
export function createProcessor(options = {}) {
  ...
  pipeline.use(() => function (ast, file) {
    if (SourceMapGenerator) {
      file.sourceMap = new SourceMapGenerator({ file: file.path || 'unknown.mdx' })
    }
  })
...
export default {
  beforeCreate () {
    Object.assign(this, meta, frontmatter)
  },
}
`

    const wrapInScript = (children, properties, extras = '') => {
      if (children.length === 0) return

      const scriptWithJSX = {
        ...children[0].data.estree,
        body: children.flatMap(node => node.data.estree.body),
      }

      // Compile any JSX to method calls.
      const script = buildJsx(scriptWithJSX, { importSource: 'iles', runtime: 'automatic' })

      let renderedScript = estreeToString(script, { sourceMap: file.sourceMap })

      // Replace any exports in script setup with local definitions.
      if (properties.setup) renderedScript = renderedScript.replace(/\bexport const\b/g, 'const')

      return {
        type: "element",
        tagName: "script",
        properties,
        children: [{ type: "text", value: `\n${renderedScript}${extras}` }],
      }
    }

    ast.children = [
      wrapInScript(scriptSetupNodes, { setup: true, lang: 'ts' }),
      wrapInScript(scriptNodes, { lang: 'ts' }, metaAssign),
      { type: 'element', tagName: 'template', children: ast.children },
    ].filter(x => x).flatMap(node => [node, br, br])
  })

  pipeline.use(vueStringify)
