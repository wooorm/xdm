...
export function createProcessor(options = {}) {
  ...
  pipeline.use(() => async (ast, file) => {
    ...
    visit(ast, node => node.name === 'template' || node.tagName === 'pre' || node.tagName === 'code' || node.type.startsWith('mdx') || node.type.includes('jsx'),  (node, index, parent) => {
      ...
      if (node.name === 'template') {
        ...
        node.content = { children: node.children }
