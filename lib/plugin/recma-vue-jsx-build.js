...
export function buildJsx(tree) {
  ...
  walk(tree, {
    ...
    leave(/** @type {Node} */ node) {
      ...
      if (!isFragment) {
        ...
        while (++index < attributes.length) {
          ...
          if (attribute.type === 'JSXSpreadAttribute') {
            ...
            vSlots = attribute.value?.expression
          }
          else {
      ...
      const slots = resolveSlots(isFragment, children, vSlots)
 ...
 * @param {JSXAttribute['value']|null} vSlots
 * @return {Expression|undefined}
 */
function resolveSlots (isFragment, children, vSlots) {
  /** @type {ArrayExpression|undefined} */
  const childrenExpr = children.length === 0
    ? undefined
    : { type: 'ArrayExpression', elements: children }

  if (isFragment) return childrenExpr

  const defaultSlot = createDefaultSlotFn(childrenExpr)
...
/**
 * @param  {Expression|undefined} body
 * @return {ObjectExpression|undefined}
 */
function createDefaultSlotFn (body) {
  if (!body) return
  return {
